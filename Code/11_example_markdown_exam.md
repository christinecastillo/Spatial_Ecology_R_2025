...
If you recieve errors, check thtat the path is correct and that the image is inside the folder 
...
The image is flipped, so I can solve the problem bu the flip() function:
```r
group <- flip(group)
im.plotRGB(group, r=1, g=2, b=3)
```
